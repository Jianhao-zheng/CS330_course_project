...
def main(args):
    ...
    if args.save_data:
        ...
        out_data = ReplayBuffer(
            STATE_DIM,
            ACTION_DIM,
            max_size=args.num_task * args.num_iter * args.max_path_length,
        )
    for task_idx in tqdm(range(args.num_task)):
        env.set_task(ml1.train_tasks[task_idx])
        ...
        for iter_idx in range(args.num_iter):
            ...
            while not truncate:
                ...
                out_data.add(state, action, next_state, reward, info["success"])
                state = next_state
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_iter", type=int, default=40, help="number of iterations to run"
    )
    parser.add_argument(
        "--num_task", type=int, default=50, help="number of tasks to run"
    )
    parser.add_argument(
        "--max_path_length",
        type=int,
